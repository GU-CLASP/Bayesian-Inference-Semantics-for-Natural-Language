...
genQProb prob (SimplePred cn) (SimplePred vp) = (areaRatio <$> cn <*> vp) :>: prob
...
-- | This version of 'every' tests direct inclusion if possible.
universalOptimized :: Pred -> VP -> Prop
universalOptimized (SimplePred b1) (SimplePred b2) = Con (liftA2 (⊆) b1 b2)
-- (liftA2 (⊆) b1 b2)
...
newInd :: P Ind
newInd = newVector (sampleUniform 0 1)

newIndSuch :: CN -> P Ind
newIndSuch cn = do
  x <- newInd
  Lib3.observe (S $ evalPred cn x)
  return x
...
newPred = do
  m <- newMeasure
  return (SimplePred (boxToVolume <$> (measureBounds m)))
...
data Pred = SimplePred (Probabilistic Volume) | EvalledPred ShallowPred

evalPred :: Pred -> ShallowPred
evalPred (EvalledPred p) = p
evalPred (SimplePred box) = \x -> Con (liftA2 (∈) (sequenceA x) box)
...
is :: Measure -> Pred
is = SimplePred . fmap boxToVolume . measureBounds
...
instance Lattice Pred where
  (SimplePred b1) /\ (SimplePred b2) = SimplePred (liftA2 (/\) b1  b2)
...
non (SimplePred vol) = SimplePred (complement <$> vol)
...
-- | Predicate: more than the given score $k$. k < 1.
moreThanScore :: Measure -> Probabilistic R -> Pred
moreThanScore Measure{..} k = SimplePred $ boxToVolume <$> measureBounds Measure{mSlopes = scale <$> mSlopes,.. }
...
universe :: Pred
universe = SimplePred $ pure $ boxToVolume $ Box $ replicate numberOfDimensions $ Ivl 0 1
