...
import Common hiding (Prop)
import qualified Common as Lib
import Lib (Ind)
...
eval (Lib2.If c x y) = Lib.If c (eval x) (eval y)
...
probablyDef v x = Lib2.If (Lib.sample (Lib.bernouilli v)) x (not' x)
