...
newtype Logit = Logit (Expr Float)
...
not' :: Prop -> Prop
-- not' x = -(log (1 - exp (-x)))
-- not' x = -log(1 - cosh(x) + sinh(x))
not' (Logit x) = Logit (-log(1-(exp(x)+exp(-x))/2+(exp(x)-exp(-x))/2))
-- try and make this numerically stable.
...
toLogits :: Expr Float -> Prop
toLogits r = Logit (negate (log r))

fromLogits :: Prop -> Expr Float
fromLogits (Logit r) = exp (negate r)



-- this is an "intuitionistic" probably (it does not exclude the strong implication)
probablyInt :: Expr Float -> Prop -> Prop
probablyInt v x = toLogits v --> x
...
greaterThan x y = _
-- toLogits (sigmoid(x-y))
-- -log(1/(1 + e^(y-x)))
-- -log(e^(x-y)/(1 + e^(x-y)))
-- y-x + log(1 + e^(x-y))
-- if x>y; log(1 + e^(y-x)) else y-x + log(1 + e^(x-y))
