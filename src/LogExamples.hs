...
ex0 = toMathematica $
  Sigma individual $ \ x ->
  Sigma predicate $ \ p ->
  IsTrue (fromS $ cltoS True (S1 (PN x) (bare (app p))))

-- >>> ex0
-- "(Integrate [(Integrate [(Integrate [(Integrate [(Integrate [(Integrate [Boole[((1-max[(abs[((0+((1-0)*x0))-(0+((1-0)*x2)))]*(1/(1 % 100+x4))),(abs[((0+((1-0)*x1))-(0+((1-0)*x3)))]*(1/(1 % 100+x5)))])>0)]*PDF[BetaDistribution[2,8]][x5], {x5,0,1}])*PDF[BetaDistribution[2,8]][x4], {x4,0,1}])*1, {x3,0,1}])*1, {x2,0,1}])*1, {x1,0,1}])*1, {x0,0,1}])"
